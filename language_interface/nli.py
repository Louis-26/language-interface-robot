...
def record_audio():
    """Record audio using spacebar controls"""
    p = pyaudio.PyAudio()
...
- Is the intent to find a path clear?
...
def main():
    ...
    while True:
        # Record audio using spacebar
        record_audio()
        start_time_stamp = time.time()
        # Transcribe audio
        command = transcribe_audio()
        print(f"\n🗣️ Command: {command}")
        transcription_time_stamp = time.time()
        transcription_time = transcription_time_stamp - start_time_stamp
        
        # Exit condition
        if command.lower() in ['exit', 'quit']:
            break
        
        # Generate action or get clarification in a single call
        print("🤖 Processing...")
        response = generate_action(command)
        
        action_time_stamp = time.time()
        action_generation_time = action_time_stamp - transcription_time_stamp
        ...
        # print(f"Latency (Transcription): {transcription_time:.2f} seconds")
        # print(f"Latency (Processing): {action_generation_time:.2f} seconds")
        # print(f"Total Latency: {transcription_time + action_generation_time:.2f} seconds")
        print(f"⏱️ Total latency: {time.time() - start_time_stamp:.2f} sec")
